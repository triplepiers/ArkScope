...
import NoticeMoreButton from '@/components/Endfield/Buttons/NoticeMoreButton.vue'
...
<template>
  <section ref="root" :key="replayKey" class="notice-carousel">
    ...
    <div class="nc-content">
      ...
      <nav class="nc-pager" aria-label="公告轮播"><RoundButton variant="notice" direction="left" :disabled="!records.length || active === 0" aria-label="上一条" @click="go(active - 1)" /><RoundButton variant="notice" direction="right" :disabled="!records.length || active === records.length - 1" aria-label="下一条" @click="go(active + 1)" /></nav>
      <NoticeMoreButton class="nc-more" :label="moreLabel" :href="moreHref" @click="emit('more')" />
    </div>
  </section>
</template>
