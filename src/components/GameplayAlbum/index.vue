...
const canvasStyle = computed(() => ({ '--section-height': `${props.sectionHeight * 8}px`, '--offset-y': `${props.offsetY}px`, '--color-bar': `url(${colorBar})` }))
  ...
  <div class="ga-viewport"><section :key="replayKey" class="gameplay-album" :class="`is-${layout}`" :style="canvasStyle">
    ...
    <div class="ga-album">
      ...
      <nav class="ga-pager ga-flash" aria-label="Carousel navigation">
        <RoundButton variant="gameplay" aria-label="Previous" :disabled="changing || slides.length < 2" @click="move(-1)" /><RoundButton variant="gameplay" direction="right" aria-label="Next" :disabled="changing || slides.length < 2" @click="move(1)" />
      </nav>
